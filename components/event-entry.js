...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    renderButtons() {
      ...
      const shareAction = navigator.share ? "share" : "clipboard";
      const sharedMsg = navigator.share ? "Compartido" : "Link copiado";
      const shareUrl = `${location.origin}?q=${encodeURI(slug)}`;

      htmlString += /*html*/ `
        <share-url action="${shareAction}" text-success="${sharedMsg}" url="${shareUrl}">
        <button part="button">
